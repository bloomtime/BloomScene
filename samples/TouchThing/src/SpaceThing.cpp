...
bool SpaceThing::touchBegan( ci::app::TouchEvent::Touch touch )
{
    return hitTest( touch.getPos() );
}

bool SpaceThing::touchEnded( ci::app::TouchEvent::Touch touch )
{
    return hitTest( touch.getPos() );
}

bool SpaceThing::hitTest( const ci::Vec2f &screenPos )
{
    Vec2f pos = globalToLocal(screenPos);
    return pos.length() <= mPlanetRadius;
}